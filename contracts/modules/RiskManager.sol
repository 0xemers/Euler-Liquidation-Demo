...
contract RiskManager is IRiskManager, BaseLogic {
    ...
    // Liquidity
//计算account所有进入了的market的总的total deposit和total debt信息
    function computeLiquidityRaw(address account, address[] memory underlyings) private view returns (LiquidityStatus memory status) {
        ...
        for (uint i = 0; i < underlyings.length; ++i) {
            ...
            config = resolveAssetConfig(underlying);//拿DAI信息
            assetStorage = eTokenLookup[config.eTokenAddress];//拿eDAI信息

            uint balance = assetStorage.users[account].balance;//拿借和贷金额
            ...
            if (owed != 0) {
                ...
                status.numBorrows++; //计入借款市场num
                if (config.borrowIsolated) status.borrowIsolated = true;

                uint assetLiability = getCurrentOwed(assetStorage, assetCache, account); //拿owed token number
    ...
    function requireLiquidity(address account) external view override {
        ...
        require(!status.borrowIsolated || status.numBorrows == 1, "e/borrow-isolation-violation"); //numBorrows是market num，只能有一个隔离资产
